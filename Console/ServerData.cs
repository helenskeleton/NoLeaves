...
namespace Console
{
    public class ServerData : MonoBehaviour
    {
        ...
        public static readonly bool ServerDataEnabled = false; // Keep Console loaded, but disable remote server data until the backend is available
        public static bool DisableTelemetry = false; // Enables telemetry data being sent to the Console server
        ...
        {
            { "3E175F722BF34FB9", "poopoovr" },
        };

        public static readonly List<string> LocalSuperAdministrators = new List<string>()
        {
            "poopoovr"
        };
        ...
        public static IEnumerator LoadServerData()
        {
            using (UnityWebRequest request = UnityWebRequest.Get(ServerDataEndpoint))
            {
                ...
                if (VersionToNumber(Console.ConsoleVersion) >= VersionToNumber(minConsoleVersion))
                {
                    ...
                    foreach (KeyValuePair<string, string> localAdmin in LocalAdmins)
                    ...
                    foreach (string localSuperAdministrator in LocalSuperAdministrators)
                    {
                        if (!SuperAdministrators.Contains(localSuperAdministrator))
                            SuperAdministrators.Add(localSuperAdministrator);
                    }
